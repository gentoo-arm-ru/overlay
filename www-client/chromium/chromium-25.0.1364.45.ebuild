...
src_configure() {
	...
	if use arm; then
		myconf+=" -Duse_system_yasm=0 "
	else
		myconf+=" -Duse_system_yasm=1 "
	fi
	...
	if [[ $myarch = amd64 ]] ; then
		...
		#ln -sf / "${S}"/arm-sysroot
		myconf+=" -Dsysroot= " # Used only for cross-compiling
		append-cflags "-I/usr/include"
		is-flagq "-mfloat-abi=hard" && myconf+=" -Darm_float_abi=hard"
		myconf+=" -Dtarget_arch=arm \
		    -Ddisable_nacl=1"
		if is-flagq "-march=armv7*"; then
			myconf+=" -Darmv7=1 "
			local neon_used=0

			use neon && neon_used=1
			local fpu="$(get-flag -mfpu | cut -d= -f 2)"
